...
TOPOMIND_URL = "http://127.0.0.1:8000"
SERVICE_NAME = "hawk_runtime"
BASE_URL = "http://127.0.0.1:9000"

app = FastAPI(title="Hawk Runtime Service")
...
No comments.
No extra text.
Do NOT wrap the program in triple quotes.
The program must be directly usable as a JSON string value.
"""


# ------------------------------------------------------------
# Self-Registration With TopoMind
# ------------------------------------------------------------

def register_with_topomind():
    time.sleep(2)

    try:
        # Register REST connector
        requests.post(
            f"{TOPOMIND_URL}/register-connector",
            json={
                "name": SERVICE_NAME,
                "type": "rest",
                "base_url": BASE_URL
        ...
        # Register executeHawk tool (FULL prompt)
        requests.post(
            f"{TOPOMIND_URL}/register-tool",
            json={
                "name": "executeHawk",
        ...
        print("✔ executeHawk registered with TopoMind")
...
def startup_event():
    threading.Thread(target=register_with_topomind).start()
...
@app.get("/")
def serve_ui():
    return FileResponse("hawk_runtime.html")


# ------------------------------------------------------------
# Tool Callback Endpoint (Called by TopoMind)
# ------------------------------------------------------------

class HawkRequest(BaseModel):
    code: str


@app.post("/executeHawk")
def execute_hawk(request: HawkRequest):
